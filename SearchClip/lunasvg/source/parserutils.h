...
inline bool readUntil(const char *&ptr, const char *end, const char ch,
                      std::string &value) {
  auto start = ptr;
...
inline bool readUntil(const char *&ptr, const char *end, const char *data,
                      std::string &value) {
  auto start = ptr;
...
inline bool skipWsDelimiter(const char *&ptr, const char *end,
...
template <typename T>
inline bool parseInteger(const char *&ptr, const char *end, T &integer,
                         int base = 10) {
  bool isNegative = 0;
  T value = 0;
  ...
  do {
    ...
    if (value > maxMultiplier ||
        (value == maxMultiplier &&
         static_cast<T>(digitValue) >
             (intMax % static_cast<T>(base)) + isNegative))
      return false;
