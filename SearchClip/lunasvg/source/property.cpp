...
Paint::Paint(const Color &color) : m_color(color) {}
...
Rect::Rect(const Box &box) : x(box.x), y(box.y), w(box.w), h(box.h) {}
...
Transform::Transform(double m00, double m10, double m01, double m11, double m02,
                     double m12)
    : m00(m00), m10(m10), m01(m01), m11(m11), m02(m02), m12(m12) {}

Transform::Transform(const Matrix &matrix)
    : m00(matrix.a), m10(matrix.b), m01(matrix.c), m11(matrix.d), m02(matrix.e),
      m12(matrix.f) {}
...
Transform Transform::operator*(const Transform &transform) const {
  ...
  double m02 =
      this->m02 * transform.m00 + this->m12 * transform.m01 + transform.m02;
  double m12 =
      this->m02 * transform.m10 + this->m12 * transform.m11 + transform.m12;
...
Transform &Transform::transform(double m00, double m10, double m01, double m11,
...
Point Transform::map(const Point &point) const { return map(point.x, point.y); }
...
Rect Transform::map(const Rect &rect) const {
  ...
  const Point p[] = {map(x1, y1), map(x2, y1), map(x2, y2), map(x1, y2)};
...
void Path::cubicTo(double x1, double y1, double x2, double y2, double x3,
                   double y3) {
...
bool Path::empty() const { return m_commands.empty(); }

void Path::quadTo(double cx, double cy, double x1, double y1, double x2,
                  double y2) {
...
void Path::arcTo(double cx, double cy, double rx, double ry,
  ...
  auto n_segs =
      static_cast<int>(std::ceil(std::fabs(th_arc / (pi * 0.5 + 0.001))));
  for (int i = 0; i < n_segs; i++) {
    ...
    auto t = (8.0 / 3.0) * std::sin(thHalf * 0.5) * std::sin(thHalf * 0.5) /
             std::sin(thHalf);
...
void Path::rect(double x, double y, double w, double h, double rx, double ry) {
  ...
  if (rx == 0.0 && ry == 0.0) {
    ...
    cubicTo(right, bottom - ry + cpy, right - rx + cpx, bottom, right - rx,
            bottom);
    lineTo(x + rx, bottom);
...
PathIterator::PathIterator(const Path &path)
    : m_commands(path.commands()), m_points(path.points().data()) {}
...
bool PathIterator::isDone() const { return (m_index >= m_commands.size()); }
...
Length::Length(double value) : m_value(value) {}
...
double Length::value(const Element *element, LengthMode mode) const {
  if (m_units == LengthUnits::Percent) {
    ...
    auto w = viewport.w;
    auto h = viewport.h;
    auto max = (mode == LengthMode::Width) ? w
               : (mode == LengthMode::Height)
                   ? h
                   : std::sqrt(w * w + h * h) / sqrt2;
    return m_value * max / 100.0;
  }
...
LengthContext::LengthContext(const Element *element) : m_element(element) {}

LengthContext::LengthContext(const Element *element, Units units)
    : m_element(element), m_units(units) {}

double LengthContext::valueForLength(const Length &length,
...
Transform PreserveAspectRatio::getMatrix(double width, double height,
                                         const Rect &viewBox) const {
  if (viewBox.empty())
    return Transform{};
  ...
  auto scale = (m_scale == MeetOrSlice::Meet) ? std::min(xscale, yscale)
...
Rect PreserveAspectRatio::getClip(double width, double height,
                                  const Rect &viewBox) const {
  if (viewBox.empty())
...
Angle::Angle(MarkerOrient type) : m_type(type) {}

Angle::Angle(double value, MarkerOrient type) : m_value(value), m_type(type) {}
