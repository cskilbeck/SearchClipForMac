...
PaintElement::PaintElement(ElementID id) : StyledElement(id) {}

GradientElement::GradientElement(ElementID id) : PaintElement(id) {}
...
std::unique_ptr<LayoutObject>
LinearGradientElement::getPainter(LayoutContext *context) const {
  ...
  while (true) {
    if (!attributes.hasGradientTransform() &&
        current->has(PropertyID::GradientTransform))
    ...
    if (!attributes.hasGradientUnits() &&
        current->has(PropertyID::GradientUnits))
    ...
    if (!ref || !(ref->id == ElementID::LinearGradient ||
                  ref->id == ElementID::RadialGradient))
      break;
...
std::unique_ptr<LayoutObject>
RadialGradientElement::getPainter(LayoutContext *context) const {
  ...
  LengthContext lengthContext(this, attributes.gradientUnits());
  gradient->cx =
      lengthContext.valueForLength(attributes.cx(), LengthMode::Width);
  gradient->cy =
      lengthContext.valueForLength(attributes.cy(), LengthMode::Height);
  gradient->r = lengthContext.valueForLength(attributes.r(), LengthMode::Both);
  gradient->fx =
      lengthContext.valueForLength(attributes.fx(), LengthMode::Width);
  gradient->fy =
      lengthContext.valueForLength(attributes.fy(), LengthMode::Height);
  return std::move(gradient);
}
...
PatternElement::PatternElement() : PaintElement(ElementID::Pattern) {}
...
std::unique_ptr<LayoutObject>
PatternElement::getPainter(LayoutContext *context) const {
  ...
  while (true) {
    ...
    if (!attributes.hasPatternTransform() &&
        current->has(PropertyID::PatternTransform))
    ...
    if (!attributes.hasPatternContentUnits() &&
        current->has(PropertyID::PatternContentUnits))
    ...
    if (!attributes.hasPreserveAspectRatio() &&
        current->has(PropertyID::PreserveAspectRatio))
  ...
  pattern->width =
      lengthContext.valueForLength(attributes.width(), LengthMode::Width);
  pattern->height =
      lengthContext.valueForLength(attributes.height(), LengthMode::Height);
...
SolidColorElement::SolidColorElement() : PaintElement(ElementID::SolidColor) {}

std::unique_ptr<LayoutObject>
SolidColorElement::getPainter(LayoutContext *) const {
  auto solid = std::make_unique<LayoutSolidColor>();
