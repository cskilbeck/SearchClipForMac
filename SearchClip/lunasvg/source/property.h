...
enum class Display { Inline, None };
...
enum class WindRule { NonZero, EvenOdd };

enum class Units { UserSpaceOnUse, ObjectBoundingBox };
...
public:
  Transform() = default;
  Transform(double m00, double m10, double m01, double m11, double m02,
            double m12);
  ...
  Transform &transform(double m00, double m10, double m01, double m11,
...
enum class PathCommand { MoveTo, LineTo, CubicTo, Close };
...
public:
  ...
  void arcTo(double cx, double cy, double rx, double ry, double xAxisRotation,
...
enum LengthMode { Width, Height, Both };
...
public:
  ...
  bool isRelative() const {
    return m_units == LengthUnits::Percent || m_units == LengthUnits::Em ||
...
enum class MarkerOrient { Auto, Angle };
