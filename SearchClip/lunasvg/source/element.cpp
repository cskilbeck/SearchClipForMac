...
void PropertyList::set(PropertyID id, const std::string &value,
                       int specificity) {
...
Element::Element(ElementID id) : id(id) {}
...
bool Element::has(PropertyID id) const { return properties.get(id); }
...
void Element::layoutChildren(LayoutContext *context,
...
Rect Element::currentViewport() const {
  ...
  if (parent->id == ElementID::Svg) {
    ...
    auto _h =
        lengthContext.valueForLength(element->height(), LengthMode::Height);
