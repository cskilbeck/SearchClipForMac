...
LayoutObject::LayoutObject(LayoutId id) : id(id) {}
...
Rect LayoutObject::map(const Rect &) const { return Rect::Invalid; }

LayoutContainer::LayoutContainer(LayoutId id) : LayoutObject(id) {}
...
LayoutObject *
LayoutContainer::addChildIfNotEmpty(std::unique_ptr<LayoutContainer> child) {
...
LayoutClipPath::LayoutClipPath() : LayoutContainer(LayoutId::ClipPath) {}
...
LayoutMask::LayoutMask() : LayoutContainer(LayoutId::Mask) {}
...
LayoutSymbol::LayoutSymbol() : LayoutContainer(LayoutId::Symbol) {}
...
Rect LayoutSymbol::map(const Rect &rect) const { return transform.map(rect); }

LayoutGroup::LayoutGroup() : LayoutContainer(LayoutId::Group) {}
...
Rect LayoutGroup::map(const Rect &rect) const { return transform.map(rect); }

LayoutMarker::LayoutMarker() : LayoutContainer(LayoutId::Marker) {}

Transform LayoutMarker::markerTransform(const Point &origin, double angle,
...
Rect LayoutMarker::markerBoundingBox(const Point &origin, double angle,
                                     double strokeWidth) const {
...
void LayoutMarker::renderMarker(RenderState &state, const Point &origin,
                                double angle, double strokeWidth) const {
  BlendInfo info{clipper, masker, opacity, clip};
  RenderState newState(this, state.mode());
  newState.transform =
      transform * markerTransform(origin, angle, strokeWidth) * state.transform;
  newState.beginGroup(state, info);
  renderChildren(newState);
  newState.endGroup(state, info);
}

LayoutPattern::LayoutPattern() : LayoutContainer(LayoutId::Pattern) {}
...
void LayoutPattern::apply(RenderState &state) const {
  ...
  renderChildren(newState);
  state.canvas->setTexture(newState.canvas.get(), TextureType::Tiled,
                           transform);
}

LayoutGradient::LayoutGradient(LayoutId id) : LayoutObject(id) {}
...
void LayoutLinearGradient::apply(RenderState &state) const {
  ...
  state.canvas->setLinearGradient(x1, y1, x2, y2, stops, spreadMethod,
                                  transform);
}
...
void LayoutRadialGradient::apply(RenderState &state) const {
  ...
  state.canvas->setRadialGradient(cx, cy, r, fx, fy, stops, spreadMethod,
                                  transform);
}

LayoutSolidColor::LayoutSolidColor() : LayoutObject(LayoutId::SolidColor) {}
...
void FillData::fill(RenderState &state, const Path &path) const {
  ...
  state.canvas->fill(path, state.transform, fillRule, BlendMode::Src_Over,
                     opacity);
}
...
void StrokeData::stroke(RenderState &state, const Path &path) const {
  ...
  state.canvas->stroke(path, state.transform, width, cap, join, miterlimit,
                       dash, BlendMode::Src_Over, opacity);
}
...
MarkerPosition::MarkerPosition(const LayoutMarker *marker, const Point &origin,
                               double angle)
    : marker(marker), origin(origin), angle(angle) {}

void MarkerData::add(const LayoutMarker *marker, const Point &origin,
                     double angle) {
...
void MarkerData::render(RenderState &state) const {
  for (const auto &position : positions)
    position.marker->renderMarker(state, position.origin, position.angle,
                                  strokeWidth);
}

void MarkerData::inflate(Rect &box) const {
  for (const auto &position : positions)
    box.unite(position.marker->markerBoundingBox(position.origin,
                                                 position.angle, strokeWidth));
}

LayoutShape::LayoutShape() : LayoutObject(LayoutId::Shape) {}
...
void LayoutShape::render(RenderState &state) const {
  ...
  if (newState.mode() == RenderMode::Display) {
    ...
    newState.canvas->fill(path, newState.transform, clipRule, BlendMode::Src,
                          1.0);
  }
...
Rect LayoutShape::map(const Rect &rect) const { return transform.map(rect); }
...
void RenderState::endGroup(RenderState &state, const BlendInfo &info) {
  ...
  state.canvas->blend(canvas.get(), BlendMode::Src_Over,
                      m_mode == RenderMode::Display ? info.opacity : 1.0);
}
...
LayoutObject *
LayoutContext::addToResourcesCache(const std::string &id,
                                   std::unique_ptr<LayoutObject> resources) {
...
LayoutClipPath *LayoutContext::getClipper(const std::string &id) {
  ...
  return static_cast<LayoutClipPath *>(
      addToResourcesCache(id, std::move(clipper)));
}
...
LayoutMarker *LayoutContext::getMarker(const std::string &id) {
  ...
  return static_cast<LayoutMarker *>(
      addToResourcesCache(id, std::move(marker)));
}
...
DashData LayoutContext::dashData(const StyledElement *element) {
  ...
  auto offset = lengthContex.valueForLength(element->stroke_dashoffset(),
                                            LengthMode::Both);
...
StrokeData LayoutContext::strokeData(const StyledElement *element) {
  ...
  strokeData.width =
      lengthContex.valueForLength(element->stroke_width(), LengthMode::Both);
...
MarkerData LayoutContext::markerData(const GeometryElement *element,
                                     const Path &path) {
  ...
  while (!it.isDone()) {
    ...
    if (!it.isDone() && (markerStart || markerMid)) {
      ...
      if (index == 1 && markerStart) {
        Point slope{outslopePoints[1].x - outslopePoints[0].x,
                    outslopePoints[1].y - outslopePoints[0].y};
      ...
      if (index > 1 && markerMid) {
        Point inslope{inslopePoints[1].x - inslopePoints[0].x,
                      inslopePoints[1].y - inslopePoints[0].y};
        Point outslope{outslopePoints[1].x - outslopePoints[0].x,
                       outslopePoints[1].y - outslopePoints[0].y};
    ...
    if (it.isDone() && markerEnd) {
      Point slope{inslopePoints[1].x - inslopePoints[0].x,
                  inslopePoints[1].y - inslopePoints[0].y};
      auto angle = 180.0 * std::atan2(slope.y, slope.x) / pi;
...
LayoutBreaker::~LayoutBreaker() { m_context->removeReference(m_element); }
