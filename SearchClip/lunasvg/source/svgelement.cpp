...
SVGElement::SVGElement() : GraphicsElement(ElementID::Svg) {}
...
std::unique_ptr<LayoutSymbol>
SVGElement::build(const TreeBuilder *builder) const {
  if (isDisplayNone())
    return nullptr;
  ...
  root->clip = isOverflowHidden() ? preserveAspectRatio.getClip(_w, _h, viewBox)
                                  : Rect::Invalid;
...
void SVGElement::layout(LayoutContext *context,
  ...
  symbol->clip = isOverflowHidden()
                     ? preserveAspectRatio.getClip(_w, _h, viewBox)
                     : Rect::Invalid;
