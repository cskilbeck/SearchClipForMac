...
Length Parser::parseLength(const std::string &string,
                           LengthNegativeValuesMode mode,
...
LengthList Parser::parseLengthList(const std::string &string,
                                   LengthNegativeValuesMode mode) {
  if (string.empty())
    return LengthList{};
...
PointList Parser::parsePointList(const std::string &string) {
  ...
  PointList values;
  while (ptr < end) {
    if (!Utils::parseNumber(ptr, end, x) || !Utils::skipWsComma(ptr, end) ||
...
Transform Parser::parseTransform(const std::string &string) {
  ...
  while (ptr < end) {
    ...
    case TransformType::Matrix:
      transform.transform(values[0], values[1], values[2], values[3], values[4],
                          values[5]);
      break;
...
Path Parser::parsePath(const std::string &string) {
  ...
  while (true) {
    ...
    if (command == 'M' || command == 'm') {
      ...
      if (!parseNumberList(ptr, end, c, 3) || !parseArcFlag(ptr, end, f[0]) ||
          !parseArcFlag(ptr, end, f[1]) || !parseNumberList(ptr, end, c + 3, 2))
        return path;
      ...
      path.arcTo(currentPoint.x, currentPoint.y, c[0], c[1], c[2], f[0], f[1],
                 c[3], c[4]);
      currentPoint.x = c[3];
      currentPoint.y = c[4];
...
Rect Parser::parseViewBox(const std::string &string) {
  ...
  double h;
  if (!Utils::parseNumber(ptr, end, x) || !Utils::skipWsComma(ptr, end) ||
      !Utils::parseNumber(ptr, end, y) || !Utils::skipWsComma(ptr, end) ||
      !Utils::parseNumber(ptr, end, w) || !Utils::skipWsComma(ptr, end) ||
...
PreserveAspectRatio
Parser::parsePreserveAspectRatio(const std::string &string) {
  if (string.empty())
...
Color Parser::parseColor(const std::string &string,
  ...
  if (Utils::skipDesc(ptr, end, "rgb(")) {
    int r, g, b;
    if (!Utils::skipWs(ptr, end) || !parseColorComponent(ptr, end, r) ||
        !Utils::skipWsComma(ptr, end) || !parseColorComponent(ptr, end, g) ||
        !Utils::skipWsComma(ptr, end) || !parseColorComponent(ptr, end, b) ||
        !Utils::skipWs(ptr, end) || !Utils::skipDesc(ptr, end, ')'))
      return defaultValue;
...
Paint Parser::parsePaint(const std::string &string,
                         const StyledElement *element,
                         const Color &defaultValue) {
  if (string.empty())
    return defaultValue;
...
Overflow Parser::parseOverflow(const std::string &string,
...
bool Parser::parseLength(const char *&ptr, const char *end, double &value,
                         LengthUnits &units, LengthNegativeValuesMode mode) {
  if (!Utils::parseNumber(ptr, end, value))
    return false;
...
bool Parser::parseNumberList(const char *&ptr, const char *end, double *values,
                             int count) {
...
bool Parser::parseColorComponent(const char *&ptr, const char *end,
                                 int &component) {
  double value = 0;
  if (!Utils::parseNumber(ptr, end, value))
    return false;
...
bool Parser::parseUrlFragment(const char *&ptr, const char *end,
                              std::string &ref) {
  if (!Utils::skipDesc(ptr, end, "url(") || !Utils::skipWs(ptr, end)) {
    return false;
  }
...
bool Parser::parseTransform(const char *&ptr, const char *end,
                            TransformType &type, double *values, int &count) {
  int required = 0;
  int optional = 0;
...
bool RuleData::match(const Element *element) const {
  ...
  while (it != end) {
    ...
    if (!match &&
        (it->combinator != SimpleSelector::Combinator::Descendant &&
...
bool RuleData::matchSimpleSelector(const SimpleSelector &selector,
...
bool RuleData::matchAttributeSelector(const AttributeSelector &selector,
                                      const Element *element) const {
  ...
  auto ends_with = [](const std::string &string, const std::string &suffix) {
    ...
    return string.compare(string.size() - suffix.size(), suffix.size(),
                          suffix) == 0;
  };
...
bool RuleData::matchPseudoClassSelector(const PseudoClassSelector &selector,
                                        const Element *element) const {
  ...
  for (auto &selector : rule.selectors) {
    ...
    for (auto &simpleSelector : selector) {
      ...
      for (auto &attributeSelector : simpleSelector.attributeSelectors) {
        specificity +=
            (attributeSelector.id == PropertyID::Id) ? 0x10000 : 0x100;
      }
    }
...
std::vector<const PropertyList *>
StyleSheet::match(const Element *element) const {
...
#define IS_NAMECHAR(c)                                                         \
  (IS_STARTNAMECHAR(c) || IS_NUM(c) || (c) == '-' || (c) == '.')
static inline bool readIdentifier(const char *&ptr, const char *end,
                                  std::string &value) {
...
static inline bool readCSSIdentifier(const char *&ptr, const char *end,
                                     std::string &value) {
...
bool CSSParser::parseSelectors(const char *&ptr, const char *end,
...
bool CSSParser::parseDeclarations(const char *&ptr, const char *end,
...
#define IS_SELECTOR_STARTNAMECHAR(c)                                           \
  (IS_CSS_STARTNAMECHAR(c) || (c) == '*' || (c) == '#' || (c) == '.' ||        \
   (c) == '[' || (c) == ':')
bool CSSParser::parseSelector(const char *&ptr, const char *end,
                              Selector &selector) {
  do {
...
bool CSSParser::parseSimpleSelector(const char *&ptr, const char *end,
                                    SimpleSelector &simpleSelector) {
  std::string name;
  if (Utils::skipDesc(ptr, end, '*'))
...
static inline bool decodeText(const char *ptr, const char *end,
                              std::string &value) {
  value.clear();
  while (ptr < end) {
    auto ch = *ptr;
